			...
			<View style={[{position:"relative",width:width},style]}>
				{(()=>{
					switch (side){
						case "top":
							return [
								<Svg key={0} height={lineWidth} width={width+lineWidth} style={{position:"absolute",top:(inset?0:-lineWidth)}}>
									<Defs>
						...
						case "bottom":
							return [
								...children || [],
								<Svg key ={children.length} height={lineWidth} width={width+lineWidth} style={{position:"absolute",bottom:(inset?-lineWidth:0)}}>
									<Defs>
